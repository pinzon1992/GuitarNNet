...
% Last Modified by GUIDE v2.5 30-Aug-2015 17:03:22
...
set(myGUIdata.playerbutton,'enable','on');
guidata(hObject, myGUIdata);
...
axis('off')
myGUIdata.maxTargetPoint = 500;%500;
%--------------
...
set(myGUIdata.parameterViewerAxis,'ytick',[3],'ytickLabel',{'level'});
set(myGUIdata.parameterViewerAxis,'xticklabel',[]);
grid on;
hold off;
myGUIdata.noteID = (-7:27+7)';
myGUIdata.noteName = {'D2';' ';'E2';'F2';' ';'G2';' ';'A2';' ';'B2';'C3';' ';'D3';' ';'E3';'F3';' ';'G3';' ';'A3'; ...
...
set(myGUIdata.playerbutton,'enable','off');
end
...
% handles    structure with handles and user data (see GUIDATA)
myGUIdata = guidata(handles.eventScopeGUI);
outFileName = ['muestra' datestr(now,30) '.wav'];
[file,path] = uiputfile(outFileName,'Guardar Sonido Capturado');
if length(file) == 1 && length(path) == 1
    if file == 0 || path == 0
        %okInd = 0;
        disp('Guardar Cancelado!');
        return;
    end;
end;
...
% --- Executes on button press in playerbutton.
function playerbutton_Callback(hObject, eventdata, handles)
% hObject    handle to playerbutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
reproductor;
end
